...
public class GreenComponentTest {
  ...
  @Test
  public void execute() {
    new GreenComponent().execute(model1);

    // test it stops at 255
    assertEquals(model1.getPixelAt(0,0).getRed(), 123);
    assertEquals(model1.getPixelAt(0,0).getGreen(), 123);
    assertEquals(model1.getPixelAt(0,0).getBlue(), 123);
  ...
  public void testInvalidConstructor() {
    try {
      ...
      new GreenComponent().execute(null);
      fail();
